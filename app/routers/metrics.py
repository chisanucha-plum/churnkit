...
async def get_metrics(service: PredictionService = Depends(get_prediction_service)):
...
@router.get("/features")
async def get_features(service: PredictionService = Depends(get_prediction_service)):
...
async def get_feature_importance(
    ...
    return FeatureImportanceResponse(
        importance=importance, top_10=dict(list(importance.items())[:10])
    )
